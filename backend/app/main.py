...
def login(body: LoginIn, db: Session=Depends(get_db)):
    ...
    if not user or not user.is_active or not pwd.verify(body.password, user.password_hash):
...
def create_user(body: UserCreate, actor=Depends(admin), db: Session=Depends(get_db)):
    ...
    temporary = "ChangeMe123!"
    u=User(email=body.email,name=body.name,role=body.role,password_hash=pwd.hash(temporary))
    db.add(u); db.flush(); audit(db,actor,"USER_CREATED","USER",u.id)
    db.commit()
...
def disable_user(user_id:int, actor=Depends(admin), db:Session=Depends(get_db)):
    ...
    u.is_active=False; audit(db,actor,"USER_DISABLED","USER",u.id); db.commit()
...
def delete_user(user_id:int, actor=Depends(admin), db:Session=Depends(get_db)):
    ...
    db.delete(u); audit(db,actor,"USER_REMOVED","USER",user_id); db.commit()
...
def create_team(body:TeamCreate,actor=Depends(admin),db:Session=Depends(get_db)):
    ...
    t=Team(name=body.name,description=body.description); db.add(t); db.flush(); audit(db,actor,"TEAM_CREATED","TEAM",t.id); db.commit()
    return t
...
def routing_keys(service_id:int, _:User=Depends(current_user),db:Session=Depends(get_db)):
    return db.scalars(select(RoutingKey).where(RoutingKey.service_id==service_id).order_by(RoutingKey.id)).all()
...
def create_key(service_id:int,body:RoutingKeyCreate,actor=Depends(admin),db:Session=Depends(get_db)):
    ...
    key="rk_"+secrets.token_urlsafe(30)
    r=RoutingKey(service_id=service_id,name=body.name,key=key); db.add(r); db.flush(); audit(db,actor,"ROUTING_KEY_CREATED","ROUTING_KEY",r.id); db.commit()
    return r
...
def make_incident(db, service_id, title, description, priority, dedupe_key, user_id=None, team_id=None, actor=None):
    if dedupe_key:
        ...
        if existing:
            event(db,existing,actor,"ALERT_DEDUPLICATED",f"Alert grouped into INC-{existing.incident_number}")
    ...
    db.add(inc); db.flush(); event(db,inc,actor,"INCIDENT_CREATED","Incident created"); return inc, True
...
def create_incident(body:IncidentCreate,actor=Depends(current_user),db:Session=Depends(get_db)):
    ...
    inc,_=make_incident(db,body.service_id,body.title,body.description,body.priority,None,body.assigned_user_id,body.assigned_team_id,actor)
    audit(db,actor,"INCIDENT_CREATED","INCIDENT",inc.id); db.commit(); return inc
...
def ingest(body:AlertCreate, db:Session=Depends(get_db)):
    ...
    inc,created=make_incident(db,r.service_id,body.title,body.message,"P1" if body.severity.lower() in ("critical","fatal") else "P2",body.dedupe_key,None,None,None)
    db.commit()
...
def assign(incident_id:int,body:AssignIn,actor=Depends(current_user),db:Session=Depends(get_db)):
    i=db.get(Incident,incident_id)
    if not i: raise HTTPException(404,"Incident not found")
    i.assigned_user_id=body.user_id; i.assigned_team_id=body.team_id
    event(db,i,actor,"ASSIGNED",f"Assigned user={body.user_id} team={body.team_id}"); audit(db,actor,"INCIDENT_ASSIGNED","INCIDENT",i.id); db.commit(); return i
...
def ack(incident_id:int,actor=Depends(current_user),db:Session=Depends(get_db)):
    i=db.get(Incident,incident_id)
    if not i: raise HTTPException(404,"Incident not found")
    i.status="ACKNOWLEDGED"; i.acknowledged_at=datetime.now(timezone.utc); event(db,i,actor,"ACKNOWLEDGED","Incident acknowledged"); db.commit(); return i
...
def resolve(incident_id:int,actor=Depends(current_user),db:Session=Depends(get_db)):
    i=db.get(Incident,incident_id)
    if not i: raise HTTPException(404,"Incident not found")
    i.status="RESOLVED"; i.resolved_at=datetime.now(timezone.utc); event(db,i,actor,"RESOLVED","Incident resolved"); db.commit(); return i
...
def reopen(incident_id:int,actor=Depends(current_user),db:Session=Depends(get_db)):
    i=db.get(Incident,incident_id)
    if not i: raise HTTPException(404,"Incident not found")
    i.status="OPEN"; i.resolved_at=None; event(db,i,actor,"REOPENED","Incident reopened"); db.commit(); return i
...
def add_note(incident_id:int,body:NoteCreate,actor=Depends(current_user),db:Session=Depends(get_db)):
    ...
    n=IncidentNote(incident_id=incident_id,author_user_id=actor.id,body=body.body); db.add(n); event(db,db.get(Incident,incident_id),actor,"NOTE_ADDED","Incident note added"); db.commit(); return n
...
def add_level(policy_id:int,body:LevelCreate,actor=Depends(admin),db:Session=Depends(get_db)):
    ...
    l=EscalationLevel(policy_id=policy_id,position=pos,target_type=body.target_type,target_id=body.target_id,delay_minutes=body.delay_minutes)
    db.add(l); audit(db,actor,"ESCALATION_LEVEL_ADDED","POLICY",policy_id); db.commit(); return l
...
def create_schedule(body:ScheduleCreate,actor=Depends(admin),db:Session=Depends(get_db)):
    s=OnCallSchedule(name=body.name,timezone=body.timezone,rotation_type=body.rotation_type,start_hour=body.start_hour,end_hour=body.end_hour)
    db.add(s); db.flush()
    for pos,uid in enumerate(body.member_ids): db.add(RotationMember(schedule_id=s.id,user_id=uid,position=pos))
...
def add_override(schedule_id:int,body:OverrideCreate,actor=Depends(admin),db:Session=Depends(get_db)):
    try:
        starts=datetime.fromisoformat(body.starts_at.replace("Z","+00:00")); ends=datetime.fromisoformat(body.ends_at.replace("Z","+00:00"))
    except Exception: raise HTTPException(400,"Invalid ISO timestamps")
    o=Override(schedule_id=schedule_id,from_user_id=body.from_user_id,to_user_id=body.to_user_id,starts_at=starts,ends_at=ends,reason=body.reason)
    db.add(o); audit(db,actor,"OVERRIDE_CREATED","SCHEDULE",schedule_id); db.commit(); return o

@app.get("/api/v1/audit")
def audit_logs(_:User=Depends(admin),db:Session=Depends(get_db)): return db.scalars(select(AuditLog).order_by(AuditLog.created_at.desc()).limit(200)).all()
